...
if ($tor_reged && $tor_info)
{
	...
	else
	{
		$template->assign_block_vars('postrow.attach.tor_reged', array(
			...
			'TOR_STATUS_BY'   => ($tor_info['checked_user_id'] && ($is_auth['auth_mod'] || $tor_status_by_for_all)) ? ('<span title="'. bb_date($tor_info['checked_time']) .'"> &middot; '. profile_url($tor_info) .' &middot; <i>'. delta_time($tor_info['checked_time']) . $lang['TOR_BACK'] .'</i></span> &middot;') : '',
